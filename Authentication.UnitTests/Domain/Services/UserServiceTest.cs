...
namespace Authentication.UnitTests.Domain.Services
{
    public sealed class UserDomainServiceTest
    {
        ...
        public async Task ShouldCreateUser()
        {
            ...
            _userValidatorMock
                .Setup(userValidator =>
                    userValidator.ValidateAsync(It.IsAny<User>(), CancellationToken.None))
                .ReturnsAsync(new ValidationResult());

            _userRepositoryMock
                .Setup(userRepository =>
                    userRepository.CreateAsync(It.IsAny<User>(), CancellationToken.None))
